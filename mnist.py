...
def conv2d(input, weight_shape, bias_shape):
    ...
    return tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(input, W, strides=[1, 1, 1, 1],
					 	padding='SAME'), b))
...
def inference(x, prob):
    ...
    with tf.variable_scope("fc"):
        ...
        fc_1_drop = tf.nn.dropout(fc_1, prob)

    with tf.variable_scope("output"):
        result = layer(fc_1_drop, [1024, 10], [10])
